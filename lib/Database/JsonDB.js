    ...
    constructor(filePath, reviver) {
        ...
        this.schema = (name, schem) => {
            ...
            let sch = (_a = class Schema {
                 ...
                 * @returns the result
                 */
                _a.find = async (obj, count, except) => {
                    let listMatch = [];
                    for (let doc of JSON.parse((await pfs.readFile(pth)).toString(), pointer.reviver)[_a.schem]) {
                        ...
                        if (match(obj ?? {}, doc) === (!except)) {
                            ...
                            if (count && count > 0)
                                count--;
                            // Check whether count = 1 and no element matches
                            if (count === 1 && listMatch !== [])
                                return doc;
                            // Add element to list
                            listMatch.push(doc);
                        }
                 ...
                 */
                _a.drop = async () => await pointer.drop(_a),
                _a);
