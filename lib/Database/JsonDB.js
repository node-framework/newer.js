...
const checkType = (type, obj) => {
    if (typeof type === 'function')
        return (getWrapper(obj) && getWrapper(obj) === type) || (obj instanceof type);
    let current = true;
    for (let e in type) {
...
export default class JsonDB {
     ...
     * @constructor
     */
    constructor(filePaths, reviver = undefined) {
        /**
         * Database data
         */
        ...
        __classPrivateFieldSet(this, _JsonDB_data, JSON.parse(fs.readFileSync(filePaths).toString(), reviver), "f");
        __classPrivateFieldSet(this, _JsonDB_data, __classPrivateFieldGet(this, _JsonDB_loadSchemas, "f").call(this, JSON.parse(fs.readFileSync(filePaths).toString(), reviver)), "f"); // Weird thing happens here so I need to fix by using the next line
