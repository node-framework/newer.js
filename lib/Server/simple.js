...
function simple(opts = {}) {
    var _a, _b, _c;
    return __asyncGenerator(this, arguments, function* simple_1() {
        let 
        ...
        while (!done && !rejectReason) {
            ...
            server.once('request', (request, response) => result({ request, response }))));
        }
        // Close the server
        if (!done)
            server.close();
        ...
        if (rejectReason)
            ...
            return yield __await(Promise.reject(rejectReason));
    });
}
